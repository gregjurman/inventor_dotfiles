...
/**
  ...
  With two-part vise rotation option
...
properties = {
  ...
  twoPartViseRotate: true, // For G68 rotation for multiple part vise layouts
  twoPartEvenAngle: 0, // Vise angle for G54, G56, etc
  twoPartOddAngle: 180, // Vise angle for G55, G57, etc
  addBlockSkipSingleVise: true // Add Block skips for anything not the first vise.
};
...
function onParameter(name, value) {
  if (name == "action") {
    if (value == "pallet-next") {
      writeBlock(mFormat.format(60));
    }
else if (value == "pallet-a") {
      writeBlock(mFormat.format(61)); 
    }
else if (value == "pallet-b") {
...
function onOpen() {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        var comment = "T" + toolFormat.format(tool.number) + " " +
          ...
          "C.RAD=" + xyzFormat.format(tool.cornerRadius);
  ...
  if(properties.twoPartViseRotate) {
    writeln("");
    writeComment("NOTE, THIS PROGRAM IS FOR TWO-PART VISE LAYOUTS");
    writeComment("G54, G56, G58 = " + properties.twoPartEvenAngle + localize('deg'));
    writeComment("G55, G57, G59 = " + properties.twoPartOddAngle + localize('deg'));
	writeln("");
  }

  // absolute coordinates and feed per min
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gPlaneModal.format(17), gFormat.format(49), gFormat.format(40));
...
function onSection() {
  ...
  if (false) {
    ...
    if (hasParameter("autodeskcam:path")) { // autodeskcam
      var path = getParameter("autodeskcam:path"); // autodeskcam
  ...
  if (makeSubprogram) { // when to do a subprogram
    ...
    var oFormat = createFormat({width:(properties.o8 ? 8 : 4), zeropad:true, decimals:0});
    if (currentSection.workOffset > 1 && properties.addBlockSkipSingleVise) {
	write("/");
    }
    writeBlock(mFormat.format(98), "P" + oFormat.format(subprogram)); // call subprogram
    var nextTool = getNextSection() ? getNextSection().getTool(): null;
    if ((nextTool && tool.number != nextTool.number) &&
	(!isFirstSection() && tool.number == getPreviousSection().getTool().number) || 
	(insertToolCall && (nextTool && tool.number != nextTool.number))) {
		writeBlock(gFormat.format(40), gFormat.format(0), zOutput.format(5.0));
    ...
    var oFormat = createFormat({width:(properties.o8 ? 8 : 4), zeropad:true, decimals:0});
    write("O" + oFormat.format(subprogram) +" ");
	...
	if (hasParameter("operation-comment")) {
  	...
  	var comment = "D=" + xyzFormat.format(tool.diameter/2.0) + " ";
	if (tool.cornerRadius > 0) {
	  comment += "C.RAD=" + xyzFormat.format(tool.cornerRadius) + " ";
	}
    if ((tool.taperAngle > 0) && (tool.taperAngle < Math.PI)) {
		comment += localize("TAPER") + "=" + taperFormat.format(tool.taperAngle) + localize("deg") +' ';
    }
  ...
  if (workOffset > 0) {
    var offsetChanged = workOffset != currentWorkOffset;
    if (workOffset > 6) {
      ...
      if (p > 300) {
        ...
        //if (offsetChanged) {
          writeBlock( + " "); // G54.1P
		if(properties.twoPartViseRotate) {
		  writeBlock(gFormat.format(54.1), "P" + p, gFormat.format(0), gFormat.format(90), xOutput.format(0), yOutput.format(0), 
		  sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4));
		} else {
		writeBlock(gFormat.format(54.1), "P" + p, gFormat.format(0), gFormat.format(90), 
		  sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4));
		}
      ...
      //if (offsetChanged) {
		if(properties.twoPartViseRotate) {
		writeBlock(gFormat.format(53 + workOffset), gFormat.format(0), gFormat.format(90), xOutput.format(0), yOutput.format(0), 
		sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4));
		} else {
		writeBlock(gFormat.format(53 + workOffset), gFormat.format(0), gFormat.format(90), 
		sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4));
		}
        currentWorkOffset = workOffset;
      //}
    }
/** START BLOCK: TWO-VISE Rotation */
    if (offsetChanged && properties.twoPartViseRotate) {
      // Rotate for two-part vise setup
      if (!(workOffset%2==0)) { // Even
        //writeBlock(gFormat.format(69)); // Cancel Rotation
		if (properties.twoPartEvenAngle != 0.0) {
			writeBlock(gFormat.format(68), xOutput.format(0), yOutput.format(0), "R" + xyzFormat.format(properties.twoPartEvenAngle));
		}
      } else {
        //writeBlock(gFormat.format(69)); // Cancel Rotation
		if (properties.twoPartOddAngle != 0.0) {
			writeBlock(gFormat.format(68), xOutput.format(0), yOutput.format(0), "R" + xyzFormat.format(properties.twoPartOddAngle));
		}
	  }
    }
    /** END BLOCK: TWO-VISE Rotation */
  }
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0), gFormat.format(currentSection.isMultiAxis() ? 43.5 : 43), hFormat.format(lengthOffset), zOutput.format(1.0));
    } else {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
    ...
    writeBlock(gFormat.format(currentSection.isMultiAxis() ? 43.5 : 43), hFormat.format(lengthOffset), zOutput.format(1.0));
    writeBlock(
      gAbsIncModal.format(90),
      gMotionModal.format(0),
...
function onClose() {
  writeBlock(gFormat.format(40), gFormat.format(0), zOutput.format(8.0));
  onCommand(COMMAND_COOLANT_OFF);
  onCommand(COMMAND_STOP_SPINDLE);
  writeln("");
  //writeBlock(gFormat.format(28), gAbsIncModal.format(91), "Z" + xyzFormat.format(0)); // retract
